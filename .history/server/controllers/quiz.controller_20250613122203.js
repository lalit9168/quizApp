...
export async function getUserScores(req, res) {
  try {
    ...
    const quizzes = await Quiz.find({ "submissions.email": email });
    const scores = [];
    ...
    console.error("Get user scores error:", err.message);
    res.status(500).json({ message: "Server error while fetching user scores" });
  }
}