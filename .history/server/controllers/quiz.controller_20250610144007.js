...
export async function createQuiz(req, res) {
  try {
    ...
    const { title, questions, duration, adminEmail, role } = req.body;
...
export async function getAllSubmissions(req, res) {
  try {
    ...
    // Decode JWT token to get the email (or other user data)
    ...
    // If the email in the query doesn't match the email in the JWT, return a forbidden error
    if (emailParam && emailParam !== userEmail) {
      return res.status(403).json({ message: "Forbidden: Emails do not match" });
    }

    // Fetch quizzes and their submissions
    const quizzes = await Quiz.find({}, "title quizCode submissions");

    // Filter submissions to only include those that match the user email
    const filteredQuizzes = quizzes.map(quiz => {
      // Filter submissions based on the email
      quiz.submissions = quiz.submissions.filter(submission => submission.email === userEmail);
      return quiz;
    });

    // Return the filtered quizzes with submissions
