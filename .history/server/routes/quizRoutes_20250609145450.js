...
// Get all quizzes with user attempt status by user email from JWT
router.get("/my-quizzes", async (req, res) => {
  ...
  try {
    ...
    // Find all quizzes where submissions include this email
    const quizzes = await Quiz.find({}, "title quizCode submissions");

    // Map quizzes and attach attempt info for the user
    const userQuizzes = quizzes.map((quiz) => {
      ...
      return {
        ...
        // You can include any other user submission info needed here
      };
    });
