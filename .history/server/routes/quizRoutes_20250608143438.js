...
router.post("/submit/:quizCode", async (req, res) => {
  const { token, selectedAnswers, address, education } = req.body;
  const { quizCode } = req.params;
  ...
  try {
    ...
    const submission = quiz.submissions.find((s) => s.email === email);
    if (!submission) return res.status(404).json({ message: "Submission not found" });

    const now = new Date();
    if (now > new Date(submission.endTime)) {
      return res.status(403).json({ message: "Time expired. Cannot submit." });
    }
    ...
    submission.selectedAnswers = selectedAnswers;
  ...
  try {
    ...
    if (existingSubmission) {
      return res.status(200).json({
        ...
        endTime: existingSubmission.endTime,
      });
    }
