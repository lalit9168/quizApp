...
router.get("/attempted-quiz", async (req, res) => {
  try {
    // Get quizzes along with submissions (assuming each submission has an 'email' field)
    const quizzes = await Quiz.find({}, "title quizCode submissions");

    // Map each quiz to include attempted user emails
    const quizzesWithAttemptedUsers = quizzes.map((quiz) => {
      // Get unique emails from submissions
      const attemptedUsers = [
        ...new Set(quiz.submissions.map((sub) => sub.email))
      ];
    ...
    res.json(quizzesWithAttemptedUsers);
  } catch (error) {
    console.error("Failed to get attempted quizzes:", error.message);
    res.status(500).json({ message: "Server error" });
  }
});
