...
router.post("/submit/:quizCode", async (req, res) => {
  ...
  const userStart = quiz.startedUsers.find((u) => u.email === email);
if (!userStart) return res.status(403).json({ message: "Quiz not started" });

const now = new Date();
if (now > new Date(userStart.endTime)) {
  return res.status(403).json({ message: "Time is up. Submission not allowed." });
}
...
// POST /api/quizzes/start/:quizCode
router.post("/start/:quizCode", async (req, res) => {
  ...
  try {
    ...
    quiz.startedUsers = quiz.startedUsers || [];

    // Check if already started
    const alreadyStarted = quiz.startedUsers.find((u) => u.email === email);
    if (alreadyStarted) {
      return res.status(200).json({ startTime: alreadyStarted.startTime, endTime: alreadyStarted.endTime });
    }

    const startTime = new Date();
    const endTime = new Date(startTime.getTime() + quiz.duration * 60000);

    quiz.startedUsers.push({
      email,
      startTime,
      endTime,
    });
    ...
    res.status(500).json({ message: "Server error during quiz start" });
  }
});
