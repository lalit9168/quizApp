...
function QuizDash() {
  ...
  const fetchUserSubmissions = async () => {
    ...
    try {
      const res = await axios.get(
        "http://localhost:5001/api/quizzes/mysubmissions",
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setUserSubmissions(res.data); // Each item: { quizCode, email, score }
    } catch (err) {
  ...
  const getUserSubmissionForQuiz = (quizCode) => {
    return userSubmissions.find((s) => s.quizCode === quizCode);
  };
    ...
    <Box p={2}>
      ...
      {loading ? (
        ...
        <Grid container spacing={2}>
          {quizzes.map((quiz) => {
            const userSubmission = getUserSubmissionForQuiz(quiz.quizCode);
            return (
                  ...
                  <CardContent>
                    ...
                    {!isAdmin && (
                      userSubmission ? (
                        <Button
                          ...
                          sx={{ mt: 1 }}
                        >
                          Attempted (Score: {userSubmission.score})
                        </Button>
                      ) : (
                        <Button
                          variant="contained"
                    ...
                    {isAdmin && (
                      <Box mt={1} display="flex" flexDirection="column" gap={1}>
                        <Button variant="outlined" onClick={() => handleDelete(quiz.quizCode)}>
                          Delete
                        </Button>
                        <Button variant="outlined" onClick={() => fetchSubmissionsForQuiz(quiz.quizCode)}>
                          View
                        </Button>
                        <Button variant="outlined" onClick={() => downloadExcelForQuiz(quiz.quizCode, quiz.title)}>
                          Download Excel
                        </Button>
                      </Box>
                    )}
