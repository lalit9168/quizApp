...
function AttemptQuiz() {
  ...
  const [startRegistered, setStartRegistered] = useState(false);
    ...
    const fetchQuiz = async () => {
      try {
        ...
        const hasAttempted = quizData.submissions?.some(
          (s) => s.email === userEmail && s.score !== undefined
        );

        if (hasAttempted) {
          setAlreadyAttempted(true);
        } else {
          // Register quiz start if not already submitted
          await axios.post(`http://localhost:5001/api/quizzes/start/${code}`, {
            token,
          });
          setStartRegistered(true);
        }

        setQuiz(quizData);
      } catch (err) {
        console.error("Quiz not found or start failed", err);
      }
    };
  ...
  const handleNext = async () => {
    if (current < quiz.questions.length - 1) {
      ...
      try {
        await axios.post(
          `http://localhost:5001/api/quizzes/submit/${code}`,
          {
            token,
            score: points,
            selectedAnswers,
