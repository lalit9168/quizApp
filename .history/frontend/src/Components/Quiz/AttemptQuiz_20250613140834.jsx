...
function AttemptQuiz() {
  ...
  try {
    ...
    downloadPDF(pdfDoc); // generate full report into doc

    const pdfBlob = pdfDoc.output("blob");
    const pdfFile = new File([pdfBlob], "scorecard.pdf", { type: "application/pdf" });

    const decodedToken = JSON.parse(atob(token.split(".")[1]));

    const formData = new FormData();
    formData.append("user_email", decodedToken.email);
    formData.append("user_name", decodedToken.name || "User");
    formData.append("quiz_title", quiz.title);
    formData.append("score", `${points}/${quiz.questions.length}`);
    ...
    await emailjs.send(
      ...
      "YOUR_TEMPLATE_ID",
      {
        ...
        user_name: decodedToken.name || "User",
