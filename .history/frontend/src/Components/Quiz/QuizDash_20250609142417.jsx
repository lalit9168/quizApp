...
function QuizDash() {
  ...
  try {
    ...
    // Annotate each quiz with whether the user has already attempted
    const updatedQuizzes = quizzesData.map((quiz) => {
      ...
      return {
        ...quiz,
        userAttempt: userSubmission ? { attempted: true, score: userSubmission.score } : null,
      };
    });
  ...
  quiz.userAttempt ? (
    <Button
      ...
      sx={{ mt: 1 }}
    >
      Attempted (View Score: {quiz.userAttempt.score})
    </Button>
  ) : (
    <Button
      variant="contained"
