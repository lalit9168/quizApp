...
function QuizDash() {
  ...
  // Add this inside the QuizDash component, near the other state declarations
const [scoresModalOpen, setScoresModalOpen] = useState(false);
const [userScores, setUserScores] = useState([]);

// Add this function to fetch all user scores
const fetchUserScores = async () => {
  try {
    const res = await api.get("api/quizzes/user-scores", {
...
// Add this to the navbar (inside the Toolbar component, next to the Logout button)
{!isAdmin && (
  <Button
    variant="outlined"
...
// Add this modal at the bottom of the component (with the other modal)
<Modal
      ...
      >
        ...
        >
          ...
          <Typography variant="body1" sx={{ opacity: 0.9 }}>
            Overview of all your quiz attempts
          </Typography>
        </Box>
          ...
          {userScores.length > 0 ? (
                  ...
                  <PieChart>
                    ...
                    >
                      {userScores.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
                          fill={getScoreColor(entry.score, entry.totalQuestions)} 
                        />
                      ))}
                    </Pie>
                    <Tooltip 
                      formatter={(value, name, props) => [
                        `${props.payload.score}/${props.payload.total}`, 
                        props.payload.name
                      ]}
                    />
                    <Legend />
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Chip
                            ...
                            sx={{
                              backgroundColor: getScoreColor(score.score, score.totalQuestions),
                              color: 'white',
                              fontWeight: '600'
                            }}
                          />
            ...
            <Box sx={{ textAlign: 'center', py: 4 }}>
              <Typography variant="body1" color="textSecondary">
                No quiz attempts found. Take some quizzes to see your performance!
              </Typography>
            </Box>
          )}
