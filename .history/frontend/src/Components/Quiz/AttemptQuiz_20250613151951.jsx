...
const submitQuiz = async () => {
  ...
  try {
    // Step 1: Submit score to backend
    ...
    // Step 2: Clear timer and local storage
    clearInterval(timerRef.current);
    localStorage.removeItem(`quizStartTime_${code}`);
    setTimeLeft(null);

    // Step 3: Generate PDF scorecard
    const pdfDoc = new jsPDF();
    downloadPDF(pdfDoc); // Fill PDF with content

    // Convert to base64 data URI (required by EmailJS)
    const pdfBase64URI = pdfDoc.output("datauristring"); // full URI: data:application/pdf;base64,...

    // Step 4: Decode JWT token to get user email & name
    const decodedToken = JSON.parse(atob(token.split(".")[1]));
    const userEmail = decodedToken?.email;
    ...
    // Step 5: Send email with PDF attachment using EmailJS
    await emailjs.send(
      "service_xsl27oo",         // Your EmailJS Service ID
      "template_g8c6hfe",        // Your EmailJS Template ID
      {
        to_email: userEmail,
        from_name: userName,
        ...
        // EmailJS requires full data URI (not just base64) in attachments
        attachments: [
          {
            name: "scorecard.pdf",
            data: pdfBase64URI,
      ...
      "amqojsEuLsl4OgVm1"        // Your EmailJS Public Key
    );

    alert("Scorecard has been emailed successfully!");
  } catch (err) {
    console.error("Score submission or email failed", err);
    alert("Something went wrong while emailing the score.");
  }
};
