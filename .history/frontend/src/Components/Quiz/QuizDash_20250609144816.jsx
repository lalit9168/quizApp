...
function QuizDash() {
  ...
  // Fetch submissions for a single quiz (admin)
  ...
  // Download submissions as Excel for a quiz (admin)
  ...
  // Initial load: fetch quizzes and decode token to check admin role
  useEffect(() => {
    fetchQuizzes();
    if (token) {
      try {
  ...
  return (
    <Box p={2}>
      ...
      {/* Admin Button: View All Submissions */}
      {isAdmin && (
      ...
      {/* Show all submissions across quizzes */}
      {showSubmissions &&
      ...
      {/* Show submissions for a single quiz */}
      {singleQuizView && (
        <Box mb={4}>
          <Typography variant="h6">
