...
function AttemptQuiz() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        const userSubmission = quizData.submissions?.find((s) => s.email === userEmail);
  ...
  if (!quiz)
    return <Typography sx={{ p: 3, color: "#e0e0e0" }}>Loading quiz...</Typography>;

  if (alreadyAttempted)
    return (
      <Box sx={{ p: 4, backgroundColor: "#121212", minHeight: "100vh", color: "#e0e0e0" }}>
  ...
  return (
    <Box sx={{ display: "flex", p: 4, gap: 4, backgroundColor: "#121212", minHeight: "100vh" }}>
      <Box sx={{ flexGrow: 1, color: "#e0e0e0" }}>
        ...
        {score === null && timeLeft !== null && (
          <Typography
            variant="h6"
            sx={{
              color: timeLeft < 60000 ? "#FF5252" : "#03DAC6",
              fontWeight: "bold",
              mb: 2,
            }}
          >
        ...
        {score === null ? (
          <Card sx={{ mt: 3, backgroundColor: "#1e1e1e", color: "#e0e0e0" }}>
            <CardContent>
