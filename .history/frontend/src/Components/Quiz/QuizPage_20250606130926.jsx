...
function QuizPage() {
  ...
  const handleSubmit = async () => {
    const token = localStorage.getItem('token'); // assumes you stored JWT here
    if (!token) {
      alert('You must be logged in as an admin to create a quiz.');
      return;
    }

    try {
      const res = await api.post('http://localhost:5001/api/quizzes/create',
        { title, questions },
        {
          headers: {
