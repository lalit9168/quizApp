...
function AttemptQuiz() {
  ...
  import emailjs from "@emailjs/browser"; // Make sure this import exists
...
const submitQuiz = async () => {
  ...
  try {
    ...
    downloadPDF(pdfDoc); // This function fills the pdfDoc with data
    const pdfBlob = pdfDoc.output("blob");
    const pdfFile = new File([pdfBlob], "scorecard.pdf", {
      type: "application/pdf",
    });

    // Step 4: Decode JWT token to get email and name
    const decodedToken = JSON.parse(atob(token.split(".")[1]));
    ...
    // Step 5: Prepare and send email via EmailJS
    await emailjs.send(
      "service_xsl27oo11", // your service ID
      "template_g8c6hfe11", // your template ID
      {
        user_email: userEmail,
        user_name: userName,
        quiz_title: quiz.title,
        score: `${points}/${quiz.questions.length}`,
        // Note: template must be configured to use `file` if attachment is required
      },
      "amqojsEuLsl4OgVm111" // your public key
    );
