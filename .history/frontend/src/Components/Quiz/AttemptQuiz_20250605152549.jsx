  ...
  const handleNext = () => {
    if (current < quiz.questions.length - 1) {
      ...
      quiz.questions.forEach((q, idx) => {
        if (CorrectAnswer[idx] === q.CorrectAnswer) points++;
      });
      setScore(points);
    }
  };
  ...
  return (
    <Box sx={{ p: 4 }}>
      ...
      {score === null ? (
        <Card sx={{ mt: 3 }}>
          <CardContent>
            <Typography variant="h6">Q{current + 1}: {question.questionText}</Typography>
            <RadioGroup
        ...
        <Box sx={{ mt: 4 }}>
          <Typography variant="h5">Your Score: {score}/{quiz.questions.length}</Typography>
        </Box>
      )}
    </Box>
  );
